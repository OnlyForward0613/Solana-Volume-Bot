...
import { BLOCKENGINE_URL, JITO_AUTH_KEYPAIR, JITO_FEE, RPC_ENDPOINT, RPC_WEBSOCKET_ENDPOINT } from "../config"
...
export async function bundle(txs: VersionedTransaction[], keypair: Keypair) {
  ...
  try {
    ...
    for (let i = 0; i < txNum; i++) {
      const upperIndex = Math.min((i + 1) * 3, txNum);
      const downIndex = i * 3
...
async function build_bundle(
  ...
  const accounts: any = await search.getTipAccounts();
  ...
  // Simulation befor sending bundle
  try {
