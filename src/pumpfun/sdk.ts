...
export class PumpFunSDK {
  ...
  async launchToken(
    ...
    try {
      ...
      let latestBlockhash = await this.connection.getLatestBlockhash("processed");
      ...
      createTx.add(createIx); // add create token instruction
      ...
      while (true) {
        ...
        if (result.confirmed) {
          // lutProviders["first"].getLookupTable(lut as PublicKey);
          break;
        }
        count++;
